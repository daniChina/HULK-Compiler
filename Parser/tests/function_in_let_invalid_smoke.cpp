...
bool expect_parse_error(
    ...
    try {
        ...
        if (message.find(message_substring) == std::string::npos) {
            std::cerr << "[FAIL] " << name << "\n"
...
int main() {
    try {
        ...
        bool ok = expect_parse_error(
            "SYN.4 rejects function declaration inside let body",
            grammar,
            ll1_table,
            "let outer=1 in function f()=>outer+1 in f();",
            "solo puede declararse como sentencia de programa");

        return ok ? 0 : 1;
    } catch (const std::exception& error) {
        std::cerr << "[FAIL] function_in_let invalid smoke threw: " << error.what() << "\n";
        return 1;
    }
}
