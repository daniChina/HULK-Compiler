...
int main() {
    try {
        ...
        ok &= expect_program_ast(
            ...
            "  x:Number = 0;\n"
            "  y:Number = 0;\n"
            "  translate(dx:Number,dy:Number) -> new Point(self.x + dx, self.y + dy);\n"
            ...
            "    x: Number = Number(0);\n"
            "    y: Number = Number(0);\n"
            "    translate(dx: Number, dy: Number) -> New(Point(Binary(GetAttr(Self, x), +, Identifier(dx)), Binary(GetAttr(Self, y), +, Identifier(dy))));\n"
            "    length() -> Call(Identifier(sqrt), [Binary(Binary(GetAttr(Self, x), *, GetAttr(Self, x)), +, Binary(GetAttr(Self, y), *, GetAttr(Self, y)))]);\n"
            "})\n"
